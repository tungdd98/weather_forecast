...
const APP = (function () {
  ...
  function getDateTime() {
    ...
    const date = `${arrDay[day]}, ${today.getDate()}/${
      today.getMonth() + 1
    }/${today.getFullYear()}`;
  ...
  async function getApiWeather(input) {
    try {
      let response = await fetch(
        `http://api.openweathermap.org/data/2.5/weather?q=${input}&APPID=${APPKEY}`
      );
  ...
  function definedProperty(data) {
    ...
    if (valTemp <= CONFIGS.temp["thấp"].max) temp = "t.thấp";
      ...
      valTemp >= CONFIGS.temp["trung bình"].min &&
      valTemp <= CONFIGS.temp["trung bình"].max
    )
      temp = "t.trung bình";
    else temp = "t.cao";
    ...
    if (valHumidity <= CONFIGS.humidity["thấp"].max) humidity = "d.thấp";
      ...
      valHumidity >= CONFIGS.humidity["trung bình"].min &&
      valHumidity <= CONFIGS.humidity["trung bình"].max
    )
      humidity = "d.trung bình";
    else humidity = "d.cao";

    if (
      valClouds >= CONFIGS.clouds["nhiều"].min &&
      valClouds <= CONFIGS.clouds["nhiều"].max
    )
      clouds = "nhiều";
    else clouds = "ít";
  ...
  function doWeather(action) {
    ...
    actions.forEach(
      (action) => (fn(`.w-do-weather .${action}`).style = "display: none")
    );
